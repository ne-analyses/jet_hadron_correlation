...
int main ( int argc, const char** argv) {
  ...
  try{
    while ( reader.NextEvent() ) {
      ...
      // and the auau particles only for correlations
      jetHadron::ConvertTStarJetVector( mbContainer, auauParticles, true, 0 );
      ...
      for ( int i = 0; i < auauParticles.size(); ++i ) {
        fastjet::PseudoJet assocParticle = auauParticles.at(i);
